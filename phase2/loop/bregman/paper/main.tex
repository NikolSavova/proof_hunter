...
\[
  ...
  \qquad x \in U .
\]
Following \cite[Definition 3.1]{BMW2010} we call $C$ \emph{right
$\D$-Chebyshev} if $\Proj_{C}(x)$ is a singleton for every $x \in U$. We
abbreviate
\[
  \Cstar := \nabla f(C).
\]
...
The verification is elementary throughout. In particular it uses no numerical
...
Nonconvex sets that are Chebyshev for Bregman distances are not new, and
neither are entropy kernels of non-full domain. The survey's own
Example~3.3 \cite{BMW2010} takes place without full domain, and there the
conclusion \emph{holds}: $\Cstar$ is convex. That is precisely why
Problem~\ref{prob:2} was worth asking. Example~3.3 shows that full domain is not
needed in every instance; it leaves open whether it can be deleted from the
general statement. Theorem~\ref{thm:main} supplies the missing half, an instance
in which the conclusion fails. Laude, Ochs and Cremers \cite{LOC2020} come
closer still, with nonconvex examples for entropy-like kernels, but their
single-valuedness results are local, holding on a neighbourhood under relative
prox-regularity, rather than at every point of $U$; they record
Problem~\ref{prob:2} as open.

So we claim neither the first nonconvex right $\D$-Chebyshev set nor the first
nonconvex entropy-Bregman example. The point of Theorem~\ref{thm:main} is
narrower, and is exactly what Problem~\ref{prob:2} asks for: a set that is right
$\D$-Chebyshev \emph{globally on $U$}, with nonconvex gradient image,
\emph{while hypothesis \textup{(b)} holds}, so that full domain is the only
hypothesis of Fact~\ref{fact:BMW} that fails.
...
domain. It says that $\dom f = X$ cannot be deleted from the \emph{universal}
statement of Fact~\ref{fact:BMW}. Nor does it identify the weakest hypothesis
that could replace full domain; see Section~\ref{sec:open}.
