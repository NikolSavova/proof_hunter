...
\title[A right Bregman--Chebyshev set with nonconvex gradient image]
{A right Bregman--Chebyshev set\\ with nonconvex gradient image}
...
\begin{abstract}
Let $f$ be a Legendre function on $X = \R^{n}$, let $U = \intr \dom f$, and let
$C \subseteq U$. Bauschke, Macklem and Wang record that if $\dom f = X$, if $C$
is closed and nonempty with $\cl \nabla f(C) \subseteq \intr \dom f^{*}$, and if
$C$ is right $\D$-Chebyshev, then $\nabla f(C)$ is convex, and they ask whether
the hypothesis $\dom f = X$ can be removed. It cannot. For $f$ the negative
entropy on $\R^{2}$ and $C = \{ (e^{t}, e^{-t^{2}}) : 1 \le t \le 2 \}$, the set
$C$ is compact and right $\D$-Chebyshev at every point of $U$, the hypothesis
$\cl \nabla f(C) \subseteq \intr \dom f^{*}$ holds, and $\nabla f(C)$ is a
parabolic arc, hence nonconvex.

The same example applies to a theorem of Luo, Meng, Wen and Yao, who show that
a variational condition (i) and the property (ii) that $C$ is a right $\D$-sun
are equivalent when $U = X$. Our $C$ is a right $\D$-sun for which (i) fails, so
$U = X$ cannot be removed from their theorem either. We also record, with
attribution to the same authors, that the hypothesis
$\cl \nabla f(C) \subseteq \intr \dom f^{*}$ is redundant.
\end{abstract}
...
\end{enumerate}
Consequently Fact~\textup{\ref{fact:BMW}} fails if hypothesis \textup{(a)} is
deleted and \textup{(b)}, \textup{(c)} retained.
\end{theorem}

Both of the remaining hypotheses hold in this example. Here $U^{*} = \R^{2}$ and
$\Cstar$ is compact, so $\cl \Cstar \subseteq U^{*}$; had that failed, the
example would delete two hypotheses at once and would not isolate (a).
Uniqueness of the projection holds at every point of $U$, including points
arbitrarily close to $\partial U$ and points far from $C$, and rests on the
single inequality $4t^{2} - 2 \ge 0$, valid for $t \ge 1/\sqrt2$. This is also
the only constraint the argument places on the interval $[a,b]$. The proofs
below use no numerical computation.

Section~\ref{sec:luo} applies the example to a theorem of Luo, Meng, Wen and Yao
\cite{LMWY2019}, Section~\ref{sec:b} records what is known about hypothesis (b),
and Section~\ref{sec:open} lists open questions.

\subsection*{Earlier examples}

Bauschke, Wang, Ye and Yuan \cite[Example 7.5]{BWYY2009}, reproduced as
Example~3.3 of the survey \cite{BMW2010}, work with
$h(u) = e^{u_{1}} + e^{u_{2}}$ and the segment
$A = \{ (\lambda, 2\lambda) : 0 \le \lambda \le 1 \}$. Since the right
projection for $f$ corresponds to the left projection for $f^{*}$
\cite[Lemma 3.2]{LMWY2019}, the curve
$\nabla h(A) = \{ (e^{\lambda}, e^{2\lambda}) \}$ is a nonconvex right
$\D$-Chebyshev set for the negative entropy, and full domain fails there too.
Its gradient image is $A$, which is convex, so the conclusion of
Fact~\ref{fact:BMW} holds and the example leaves Problem~\ref{prob:2} untouched.
Theorem~\ref{thm:main} replaces the convex segment $A$ by the nonconvex arc
$\{ (t, -t^{2}) \}$; the work lies in showing that Chebyshevness survives on all
of $U$.

On the dual side, Theorem~\ref{thm:main} states that the arc
$\{ (t,-t^{2}) : a \le t \le b \}$ is a compact nonconvex set that is left
Chebyshev for the Bregman distance of $h$ at every point of $\R^{2}$. Since
$\dom f = X$ holds if and only if $f^{*}$ is supercoercive, this is also the
statement that supercoercivity cannot be removed from the corresponding
left-projection theorem.

We do not claim the first nonconvex right $\D$-Chebyshev set for the negative
entropy, which is \cite[Example 7.5]{BWYY2009}, nor the duality on which the
construction rests. Laude, Ochs and Cremers \cite{LOC2020} give nonconvex
examples for entropy-like kernels, with single-valuedness on a neighbourhood
under relative prox-regularity rather than at every point of $U$; they record
Problem~\ref{prob:2} as open. Themelis and Wang \cite{TW2025} develop Bregman
operators on their natural domains.

What Theorem~\ref{thm:main} adds is a set that is right $\D$-Chebyshev on all of
$U$, with nonconvex gradient image, and with hypothesis (b) intact, so that (a)
is the only hypothesis of Fact~\ref{fact:BMW} that fails. We know of no earlier
example with these three properties together.

Theorem~\ref{thm:main} concerns the statement of Fact~\ref{fact:BMW}, not the
individual instance. Convexity of $\Cstar$ occurs for many $f$ of non-full
domain, \cite[Example 7.5]{BWYY2009} among them; what fails is the implication
once (a) is deleted. The theorem also says nothing about which weaker hypothesis
might serve in place of (a), for which see Section~\ref{sec:open}.
...
Only positivity is used below. On $[1,2]$ the infimum of
$t \mapsto e^{t} + (4t^{2} - 2)e^{-t^{2}}$ is attained at $t = 1$ and equals
$e + 2/e = 3.45404\ldots$, so that $h_{x}'' > e + 2/e$ there, uniformly in
$x \in U$. Establishing this sharper bound requires a monotonicity argument;
Lemma~\ref{lem:convex} does not.
\end{remark}
...
We collect the hypotheses and their status. Here $X$ is finite dimensional, $f$
is Legendre, $C$ is nonempty, compact and contained in $U$,
$\cl \Cstar \subseteq U^{*}$, and $C$ is right $\D$-Chebyshev on all of $U$. The
only failure is the intended one, $\dom f \neq X$. If one reads
...
\section{A consequence for a theorem of Luo, Meng, Wen and Yao}\label{sec:luo}
...
Our example satisfies (ii) but not (i), and $U \neq X$. We verify both directly
rather than by appeal to their theorems, since what is at issue is a
configuration their Theorem~3.12(2) excludes.
...
\]
is affine in $w$, and $h_{y}'(s) = 0$, since substituting
...
Corollary~\ref{cor:luo} says that $U = X$ cannot be struck from
Theorem~3.12(2); it does not say that $U = X$ is the weakest hypothesis that
would serve there, and some weaker condition on the rays $z_{\lambda}$ may well
suffice.

\begin{remark}\label{rem:obstruction}
The proof of (ii) $\Rightarrow$ (i) in \cite[Theorem 3.12(2)]{LMWY2019} applies
the sun property along $z_{\lambda}$ for arbitrarily large $\lambda$, divides by
$\lambda$, and lets $\lambda \to \infty$. For the point $x$ of
Lemma~\ref{lem:i-fails} that limit is unavailable: writing $\eta = e^{-s^{2}}$,
we have $(z_{\lambda})_{2} = \lambda \tfrac{\eta}{2} + (1-\lambda)\eta =
\eta\bigl( 1 - \tfrac{\lambda}{2} \bigr)$, so $z_{\lambda} \in U$ exactly for
$0 \le \lambda < 2$. What $U = X$ supplies in that proof is the unbounded reach
of the ray, not the sun property.
\end{remark}
...
Theorem~3.13 also applies to our example, once one checks that negative entropy
is totally convex on $U$ and that $f^{*}$ is locally uniformly totally convex on
$U^{*}$, and yields Lemma~\ref{lem:sun} again. Their Corollary~3.14, which
concludes convexity of
...
\begin{remark}
Both \cite[Fact 3.2]{BMW2010} and \cite[Theorem 3.12(2)]{LMWY2019} reach
convexity of $\nabla f(C)$ from a Chebyshev-type hypothesis, and both require
full domain at the corresponding step, one by assuming $\dom f = X$ outright and
the other by requiring $U = X$. Theorem~\ref{thm:main} and
Corollary~\ref{cor:luo} show that the requirement is not an artefact of either
proof.
\end{remark}

\section{The remaining hypothesis}\label{sec:b}

Hypothesis (b) of Fact~\ref{fact:BMW} can be removed altogether. The following
is due to Luo, Meng, Wen and Yao; we record it here because it completes the
account of Fact~\ref{fact:BMW}, and we make no claim to it.
...
Thus hypothesis (a) cannot be deleted, and hypothesis (b) need not have been
assumed.

\section{Open questions}\label{sec:open}

\begin{enumerate}[leftmargin=2.2em]
\item \emph{What replaces full domain?} Theorem~\ref{thm:main} shows that
$\dom f = X$ cannot be deleted, but not what the weakest sufficient hypothesis
is. Since the right projection onto $C$ for $f$ is the left projection onto
$\Cstar$ for $f^{*}$ \cite[Lemma 3.2]{LMWY2019}, and the obstruction here lies
on the dual side, a condition on how $\Cstar$ approaches $\partial U^{*}$ seems
the natural candidate.

\item \emph{Problem 4 of \cite{BMW2010}} asks for a characterisation of the
right $\D$-Chebyshev sets for the negative entropy, and remains open. A solution
would subsume Theorem~\ref{thm:main}, which exhibits one such set and computes
its gradient image. The example belongs to a two-parameter family, one set for
each $1/\sqrt2 \le a < b$, so such sets are not scarce.

\item \emph{Dimension and geometry.} Our example is planar and $C$ is an arc. We
do not know whether the failure can be made worse, for instance with $\Cstar$ of
nonempty interior in its convex hull, nor what happens in $\R^{n}$ for
$n \ge 3$.
\end{enumerate}

\section*{Verification}

The statements above were also checked numerically. In directed-rounding
interval arithmetic at $40$ digits, one harness verifies that $\D$ agrees with
its definition and with the generalised Kullback--Leibler form; that the
reduction of Lemma~\ref{lem:reduce} holds with constant $f(x)$; that $h_{x}''$
is bounded below uniformly on $[1,2]$; that the minimiser of $h_{x}$ is unique
for a spread of $x \in U$ including near-boundary and far-field points; and the
nonconvexity witness of Lemma~\ref{lem:nonconvex}. A second harness tests
Lemma~\ref{lem:sun} against the definition of a sun over $1{,}038$ pairs
$(x,\lambda)$, with $\lambda$ ranging past $x$ to the boundary of $U$ and with
$x$ chosen in part to project to the interior of the arc; the projection
parameter varies by at most $4 \times 10^{-40}$. It also confirms the closed
form $(s-t)^{2}\bigl(e^{-s^{2}} - x_{2}\bigr)$ of
Lemma~\ref{lem:i-fails} and its positivity. These checks are finite; the
quantifiers are carried by the proofs.
