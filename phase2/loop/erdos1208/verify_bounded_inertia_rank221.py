#!/usr/bin/env python3
"""Certificate for the rank-221 bounded-inertia Erdős 1208 bound."""
...
INERTIA_CAPS = RANK + 1
USEFUL_COUNT = 11_767
ALPHA = Decimal(493_714) / Decimal(1_000_000)  # 0.493714
W0 = Decimal(84_899)
NUMERICAL_ALLOWANCE = Decimal("1e-25")

# The positive-disk packing constant is 4/pi.  The rational lower bound
# pi>333/106 gives the safe numerical upper bound 4/pi<424/333.
...
def main() -> None:
    ...
    assert len(ramified) == INERTIA_CAPS
    assert len(useful) == USEFUL_COUNT
    assert not rejected
    assert not ramified_set.intersection(useful)

    relation_bound = RANK + INERTIA_CAPS + USEFUL_COUNT
    ...
    # Every ramified inertia group has order at most two, hence its tame
    # normalized different exponent is at most 1/2.
    ...
    # General all-depth residue-degree comparison used by the master proof.
    ...
    print("rejected useful candidates:", len(rejected))
    ...
    print("left/right active slopes:", left_slope, right_slope)
    print("right / maximum fourth slopes:", right_slope, maximum_fourth_slope)
    print("certified margins:", *margins)
    print("safe disk packing constant:", PACKING_CONSTANT_UPPER)
    print("target F_2(n) << n^0.493714: CERTIFIED")
