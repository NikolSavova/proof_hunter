...
def hybrid_profile(
    ...
    normal_head_degrees: Counter[tuple[int, Point, Point]] = Counter()
    ...
    for (base, ordinary_sum), fibre in fibres.items():
        ...
        for (
            ...
            if degenerate:
                ...
                swapped_endpoint = add(base, shift)
                swapped_midpoint_difference = subtract(
                    midpoints[base], midpoints[swapped_endpoint]
                )
                swapped_displacement = subtract(base, swapped_endpoint)
                swapped_degenerate = swapped_midpoint_difference in (
                    swapped_displacement,
                    negate(swapped_displacement),
                )
                swapped_left = subtract(
                    w_value, linear(other_shift)
                )
                swapped_right = subtract(w_value, other_shift)
                normal_key = (
                ...
                normal_head_degrees[
                    (normal_key[0], normal_key[1], swapped_endpoint)
                ] += 1
                charge = None
    ...
    for (
        ...
        candidates = [
            (left_degree, 0, normal_key[0], fixed_left, normal_key[1]),
            (right_degree, 1, normal_key[0], fixed_right, normal_key[1]),
            (
                normal_head_degrees[
                    (normal_key[0], normal_key[1], swapped_endpoint)
                ],
        ...
        if not swapped_degenerate:
            swapped_key = (
                int(antipodal_sign(swapped_endpoint)),
                swapped_midpoint_difference,
            )
            swapped_left_degree = normal_degrees[
                (0, *swapped_key, swapped_left)
            ]
            swapped_right_degree = normal_degrees[
                (1, *swapped_key, swapped_right)
            ]
            assert swapped_left_degree and swapped_right_degree
            swapped_head_degree = normal_head_degrees[
                (swapped_key[0], swapped_key[1], other_endpoint)
            ]
        ...
        chosen_degree, route, sign, anchor, h_value = min(candidates)
        six_anchor_degree_moment += chosen_degree
        charge = route, sign, anchor, h_value
    ...
    normal_charge_second_moment = sum(
        ...
        if charge[0] in (0, 1, 2, 3, 4, 5)
    )
    assert normal_charge_second_moment <= six_anchor_degree_moment
    assert six_anchor_degree_moment <= balanced_degree_moment
    ...
    for charge, preimages in first_preimages.items():
        ...
        if charge[0] in (4, 5):
            fixed_head = charge[2]
            if charge[0] == 5:
                first_q, first_p = other_shift, shift
                second_q, second_p = second_other, second_shift
            else:
                first_q, first_p = shift, other_shift
                second_q, second_p = second_shift, second_other

            def head_forms(
                w_parameter: Point, q_value: Point, p_value: Point
            ):
                a_value = subtract(fixed_head, q_value)
                return (
                    a_value,
                    fixed_head,
                    add(a_value, p_value),
                    subtract(w_parameter, q_value),
                    subtract(w_parameter, p_value),
                    subtract(w_parameter, linear(q_value)),
                    subtract(w_parameter, linear(p_value)),
                )

            first_forms = head_forms(w_value, first_q, first_p)
            second_forms = head_forms(second_w, second_q, second_p)
            rho = subtract(second_q, first_q)
            pi = subtract(second_p, first_p)
            kappa = subtract(second_w, w_value)
            expected = (
                negate(rho),
                (0, 0),
                subtract(pi, rho),
                subtract(kappa, rho),
                subtract(kappa, pi),
                subtract(kappa, linear(rho)),
                subtract(kappa, linear(pi)),
            )
            actual = tuple(
                subtract(second, first)
                for first, second in zip(first_forms, second_forms)
        ...
        if charge[0] in (1, 3):
            ...
            first_forms = right_forms(base, first_q, first_p)
            second_forms = right_forms(
                second_base, second_q, second_p
            )
        ...
        first_forms = forms(base, first_q, first_p)
        second_forms = forms(second_base, second_q, second_p)
        eta = subtract(second_base, base)
        rho = subtract(second_q, first_q)
        pi = subtract(second_p, first_p)
        expected = (
...
def main() -> None:
    ...
    expected: dict[str, Profile] = {
        ...
        "closure-40": (370_516, 369_663, 372_254, 4),
        "Costas-11": (2_264, 2_264, 2_264, 1),
        "Costas-17": (20_014, 19_858, 20_326, 2),
        "Costas-23": (498_674, 493_947, 508_164, 3),
        "Costas-31": (765_102, 751_104, 794_022, 4),
    }
    ...
    if "--extended" in sys.argv:
        ...
        expected["closure-80"] = (357_094, 356_998, 357_286, 2)
        primes += [37, 41, 43]
        expected.update(
            {
                "Costas-37": (2_939_312, 2_900_964, 3_016_296, 3),
                "Costas-41": (4_629_690, 4_525_615, 4_843_428, 4),
                "Costas-43": (8_451_318, 8_271_632, 8_815_150, 4),
            }
        )
    for prime in primes:
        matrix, _ = ROWS[prime]
