...
constexpr std::size_t kLocalPrecomputeLimit = 16000;
...
std::vector<LocalData> precompute_norm_data() {
  std::vector<LocalData> locals(kNormLimit + 1);
  for (int q = 2; q <= kNormLimit; ++q) {
    LocalData row;
    row.cost = 0.5 * std::log(q);
    const double parameter = 1.0 / (static_cast<double>(q) * q);
    ...
    for (int depth = 0; depth < 3; ++depth) {
      ...
      sum += power;
      row.gain[depth] =
          0.25 * std::log(((depth + 2.0) / sum) /
                          ((depth + 1.0) / previous_sum));
      row.slope[depth] = row.gain[depth] / row.cost;
    }
    locals[q] = row;
  }
  return locals;
...
Frontier build_frontier(const std::vector<int>& ideals,
                        const std::vector<LocalData>& locals, int start,
                        int count) {
  Frontier frontier;
  ...
  for (int step = 0; step < 3 * count; ++step) {
    ...
    for (int depth_index = 0; depth_index < 3; ++depth_index) {
      ...
      const double slope =
          locals[ideals[next[depth_index]]].slope[depth_index];
    ...
    assert(chosen >= 0);
    const LocalData& row = locals[ideals[next[chosen]++]];
    const double cost = row.cost;
    const double gain = row.gain[chosen];
...
Score score_configuration(int D, const std::vector<int>& ideals,
                          const std::vector<LocalData>& locals,
                          const std::vector<double>& prefix_cost, int t,
                          int genus_bonus) {
  ...
  if (useful <= 0 || t + useful > static_cast<int>(ideals.size()) ||
      t + useful > static_cast<int>(kLocalPrecomputeLimit)) {
    return {-std::numeric_limits<double>::infinity(), 0.0, t, d, useful};
  }
  const double log_rd = 0.5 * std::log(D) + 0.5 * prefix_cost[t];
  const Frontier frontier = build_frontier(ideals, locals, t, useful);
  auto rhs = [&](double w) {
...
int main() {
  ...
  const std::vector<LocalData> locals = precompute_norm_data();
  ...
  for (int D = kDiscriminantMin; D <= kDiscriminantLimit; ++D) {
    ...
    for (std::size_t i = 0; i < local_count; ++i) {
      prefix_cost[i + 1] = prefix_cost[i] + locals[ideals[i]].cost;
    }
    ...
    for (int t = kBroadTMin; t <= kBroadTMax; t += kBroadTStep) {
      const Score score = score_configuration(
          D, ideals, locals, prefix_cost, t, genus_bonus);
