...
data are finite and accompanied by independently reproducible certificates.
\end{abstract}
...
The number-field input follows the same tower-cutting framework used by
Hajir, Maire, and Ramakrishna~\cite{HajirMaireRamakrishna2021} and in the
recent number-field construction for the unit-distance
problem~\cite{AlonEtAl2026}.  We quote the standard tame, totally real
Shafarevich presentation theorem in the precise form needed below.  The
field-specific calculations use PARI/GP~\cite{PARI2}; the accompanying
verifier reconstructs the relevant Kummer kernel, checks every selected
prime, and evaluates the final inequalities at two independent precisions.
...
\[
 ...
 \qquad
 \operatorname{Frob}_{\mathfrak q}^2=1\quad(\mathfrak q\in U),         \tag{3.1}\label{eq:square-caps}
\]
where $\tau_{\mathfrak p}$ generates tame pro-$2$ inertia.  If
\[
 4(r_0+|T|+|U|)<d^2,                              \tag{3.2}\label{eq:gs-condition}
\]
...
groups above $\mathfrak p$.  Every relation in \eqref{eq:square-caps} is a square and
hence lies in the Frattini subgroup, so the generator rank remains $d$.
...
The standard tame Shafarevich presentation theorem, in the form used in
\cite{AlonEtAl2026,Koch2002}, computes $d$ from the totally positive
$T$-unit squareclasses that are squares modulo $4$, and gives
\[
 r_0\le d+1                                             \tag{3.3}\label{eq:shaf-bound}
\]
for the real quadratic fields used here.  We shall verify the squareclass
space explicitly rather than invoke a generic rank estimate.
...
\[
 D_0=\rd(E)\prod_{\mathfrak p\in T}
              \Nm_E(\mathfrak p)^{1/(2[E:\Q])}.         \tag{3.4}
\]
For a real quadratic base, the exponent is $1/4$.

We also need the Frobenius caps to make primes split in the Eisenstein CM
extension of each layer.  Let $Q=\Nm_E\mathfrak q$.  If $Q\equiv1\pmod3$,
this is automatic.  If $Q\equiv2\pmod3$, require the Frobenius class of
$\mathfrak q$ to be nonzero in the Frattini quotient.  It then has exact
order two after the square cap, and the residue field in the selected
layers has size $Q^2\equiv1\pmod3$.
...
\]
Let $T$ be the first $217$ odd prime ideals of $E$, ordered by norm with
the deterministic tie order used in the certificate.
...
PARI/GP.  They are not used as opaque numerical output: the verifier expands
...
rank-four row space.  For every candidate norm congruent to two modulo
...
\[
 ...
 w_0=39928.00429278135001166535408\ldots,          \tag{5.6}
\]
the margins in \eqref{eq:frontier} at $w_0$ and $2w_0$ are respectively
\[
 0.001632817036815572250219059\ldots,
 \qquad
 0.003247075212037706995029834\ldots.              \tag{5.7}\label{eq:margins}
\]
...
\item the integer Golod--Shafarevich inequality and root discriminant;
