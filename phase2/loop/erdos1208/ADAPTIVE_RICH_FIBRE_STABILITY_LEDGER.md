...
The lattice need not be `J`-stable, but the exact complete oblique case is
now also closed.  `OBLIQUE_LATTICE_GAUSSIAN_CORE.md` first gives the
independent preliminary estimate

\[
 M\gg\max\{rB,(r/B)^{3/2}\}\gg r^{6/5}.          \tag{4.3}
\]

and then proves the sharp modular-midpoint theorem

\[
 M\gg r^{3/2}.                                    \tag{4.4}
\]

Consequently every complete exact integral oblique `r`-by-`s` rich-fibre
patch satisfies

\[
 |Q|\ll M^{2/3}\sqrt S.                           \tag{4.5}
\]

The remaining gate is no longer exact lattice conditioning.  It is the
extraction of complete structure, or a stable approximate/sparse substitute,
from general seven-incidence mass and the aggregation of (2.5).
