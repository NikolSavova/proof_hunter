#!/usr/bin/env python3
"""Certificate for the rank-715 all-depth Frobenius Erdős 1208 bound."""
...
RANK = 715
USEFUL_COUNT = 127_091
ALPHA = Decimal(247293) / Decimal(500000)  # 0.494586
W0 = Decimal(1_040_100)
...
def main() -> None:
    ...
    assert len(ramified) == RANK + 1 and ramified[-1] == 5431
    assert len(useful) == USEFUL_COUNT and useful[-1] == 1_695_439
    assert not rejected
    assert not ramified_set.intersection(useful)

    relation_bound = RANK + USEFUL_COUNT
    assert relation_bound == 127_806
    assert 4 * relation_bound < RANK * RANK
    ...
    # Prefix compatibility: no depth can appear before its predecessor.
    ...
    print("target F_2(n) << n^0.494586: CERTIFIED")
