...
### 2.3 A commuting-square box norm

The four vertices in (2.10) are generated by two explicit partial maps.
For fixed `u`, put

\[
 \mathsf H_u(r,A,B)=(r-u,A+u,B).                  \tag{2.21}
\]

This preserves the first terminal `A+r` and the second start `B`.  On the
slice `B=ell`, define

\[
 \mathsf V_{c,\ell}(r,A,\ell)
 =\bigl(c-A,\ A,\ \ell+L(A+r-c)\bigr).           \tag{2.22}
\]

For `v=(p,X,ell)`, these maps give

\[
\begin{array}{c|c}
v&(p,X,\ell)\\
\mathsf H_uv&(p-u,X+u,\ell)\\
\mathsf V_{c,\ell}v&(q,X,W)\\
\mathsf H_u\mathsf V_{c,\ell}v&(q-u,X+u,W),
\end{array}                                      \tag{2.23}
\]

and a direct calculation gives the commuting identity

\[
 \boxed{\mathsf V_{c,\ell}\mathsf H_uv
       =\mathsf H_u\mathsf V_{c,\ell}v}          \tag{2.24}
\]

whenever the expressions are defined.  Consequently
`Lambda_2(C,x,u)` is exactly the endpoint-restricted box count

\[
 \sum_{v=(r,A,\ell)}
 1_{\mathcal V_K}(v)
 1_{\mathcal V_K}(\mathsf H_uv)
 1_{\mathcal V_K}(\mathsf V_{c,\ell}v)
 1_{\mathcal V_K}(\mathsf H_u\mathsf V_{c,\ell}v)
 1_{\{x\in\partial(A+r)\cup
          \partial(\ell+L(A+r-c))\}}.            \tag{2.25}
\]

This is a literal box norm inside the original tail reservoir, not a
metaphor.  A density increment should therefore regularize the fibres of
the two commuting maps while retaining the final physical-endpoint
indicator.  Dropping that indicator returns to the large abstract popular-
set energies already ruled out by counterexamples.
...
commuting-square identity (2.21)--(2.25), the product injection, all four endpoint-role Sidon
