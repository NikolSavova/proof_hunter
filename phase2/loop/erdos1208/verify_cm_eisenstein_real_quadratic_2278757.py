#!/usr/bin/env python3
"""Candidate certificate for the CM/Eisenstein Q(sqrt(2278757)) record."""
...
ideals=vecsort(Vec(ideals),[1,2]);S=ideals[1..{RAMIFIED_IDEAL_COUNT}];su=bnfsunit(bnf,vector(#S,i,S[i][3]));units=bnfunits(bnf,vector(#S,i,S[i][3]));bid=idealstar(nf,[4,[1,1]],1,2);mm=Mat(vector(#units[1],i,ideallog(nf,units[1][i],bid)));lastroot=lift(nfmodpr(nf,Mod(x,nf.pol),S[#S][3]));
print("META,",cert,",",bnf.no,",",su[5][1],",",#su[5][2],",",#units[1],",",matrank(Mod(mm,2)),",",#units[1]-matrank(Mod(mm,2)),",",S[#S][1],",",lastroot);
for(j=1,#units[1],z=nffactorback(nf,units[1][j]);if(type(z)=="t_INT",print(z,",0"),print(z[1],",",z[2])));
"""
    output = subprocess.check_output(
        [gp, "-fq", "-s", "2G"], input=script, text=True, timeout=240
    )
...
def main() -> None:
    ...
    assert ramified_ideals[-1] == (1_109, 1_109, "split", 152)
    ...
    assert metadata == (1, 2, 1, 0, 225, 4, 221, 1_109, 152)
    assert len(s_units) == 225
    ...
    assert useful_ideals[-1] == (128_591, 128_591, "split", 112_589)
    ...
    print("class / localized class / S-unit columns:", metadata[1:5])
    print("ray rank / d:", metadata[5], metadata[6])
    print(
    ...
    print("CM quadratic-2278757 F_2(n) << n^0.49368818: CERTIFIED")
