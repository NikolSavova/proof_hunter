...
\author{}
\date{\today\thanks{This investigation was carried out with substantial
...
we prove that every mirror-decomposable point set has logarithmic
convex-subset count at least
$(\log_2N)^2/2-O((\log N)^{3/2})$.  Thus the
optimal universal coefficient $1/2$ is attained within the entire
decomposable class.
\end{abstract}
...
Baek--Balko~\cite{BaekBalko2026}.  Recursively separated point sets were
studied under the name \emph{decomposable} by Balko, Kyn\v{c}l, Langerman,
and Pilz~\cite{BalkoEtAl2017}.  Baek and Balko proved the Erd\H{o}s--Szekeres
conjecture for that class~\cite[Theorem~7]{BaekBalko2026}.  Our final result
is enumerative: in the mirrored convention we determine the asymptotic
minimum total number of convex subsets in the same class.

Exact weighted identities for numbers of
...
We also determine the asymptotic answer throughout the class of
mirror-decomposable point sets; see \cref{thm:strong-class}.
...
A suitable shear $(x,y)\mapsto(x,y+Mx)$ preserves orientations
...
The same endpoint-cluster phenomenon is used by Baek and
Balko~\cite[Lemma~14]{BaekBalko2026} to control the largest convex subset of
an almost-vertical blow-up.  In the present uniform setting, retaining all
choices gives the following exact identities.
...
cup--cap construction.  Write $A\prec B$ when every point of $A$ precedes
every point of $B$ in $x$-order, internal orientations are preserved, and
every increasing triple meeting both sides has sign $-$ if its first two
points lie in $A$, and sign $+$ if its last two points lie in $B$.
...
Up to the reflection $\rho(x,y)=(-x,y)$, they are the classical
Erd\H{o}s--Szekeres sets $P(i+2,m-i+2)$ in the notation of Baek and
Balko~\cite[Section~4]{BaekBalko2026}.
...
\]
The right-hand side is strictly larger than $1/2$ for every finite $k$,
since $\binom{2k-4}{k-2}<2^{2k-4}$, and tends to $1/2$ from above.
...
\section{Optimality for decomposable point sets}

Following Balko, Kyn\v{c}l, Langerman, and Pilz~\cite{BalkoEtAl2017}, a
point set is \emph{decomposable} if it is a singleton, or if it has a
partition into decomposable sets $L$ and $R$ such that $L$ lies to the left
of and deep below $R$.  Here deep below means that every point of $L$ lies
below every line through two points of $R$, while every point of $R$ lies
above every line through two points of $L$.

We use the mirrored convention.  If $L$ lies to the left of and deep below
$R$, then for $\rho(x,y)=(-x,y)$ one has $\rho(R)\prec\rho(L)$.
Conversely, reflecting a split $A\prec B$ gives such a deep-below split.
The reflection $\rho$ preserves cap and cup sizes and the number of convex
subsets.  We therefore call a point set \emph{mirror-decomposable} if it is
a singleton, or if it has a realization $A\prec B$ in which $A$ and $B$ are
mirror-decomposable.  These are precisely the $\rho$-images of decomposable
sets.  Equivalently, they are represented by ordered full binary trees whose
internal nodes are the operation in \cref{lem:strong-glue}.

Baek and Balko's theorem controls the largest convex subset in this
class~\cite[Theorem~7]{BaekBalko2026}; the following theorem instead
determines the asymptotic minimum of the total convex-subset count.
Let
\[
...
Every $N$-point mirror-decomposable set $P$ satisfies
\begin{equation}
...
Every $s$-point mirror-decomposable set $S$ satisfies
\[
...
constructed from the Pascal templates is mirror-decomposable.  Indeed,
...
\href{https://doi.org/10.1016/j.jcta.2026.106195}
{doi:10.1016/j.jcta.2026.106195}.

\bibitem[BKLP17]{BalkoEtAl2017}
M.~Balko, J.~Kyn\v{c}l, S.~Langerman, and A.~Pilz.
Induced Ramsey-type results and binary predicates for point sets.
\emph{Electron. J. Combin.}, 24(4):Paper No.~P4.24, 22 pp., 2017.
\href{https://doi.org/10.37236/7039}{doi:10.37236/7039}.
