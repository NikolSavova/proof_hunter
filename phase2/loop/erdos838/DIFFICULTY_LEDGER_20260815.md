...
| P1g | Binary drop-charged endpoint floor and a graded reset consequence | the local inequality would give `log(X_TY_T)>=((log n)^2-log n)/2` for every binary strong tree; to affect 838 it must further imply an averaged positive-rank interval or `v_k(4^k)>=2^((1+eta-o(1))k^2)` for some `eta>0` | ROUTE unless the graded implication is proved; strictly weaker than P1 because it concerns only strong trees and initially only total endpoint profiles | Open, exact local lemma under attack | Must survive balanced glues (asymptotic equality), alternating combs, coherent endpoint ramps, and Boolean-core rank concentration.  Kill after one propagation attempt if it only improves total `V` or a partition function without forcing a positive-rank interval; then bank the endpoint theorem and return to P1b/minimizer mutation |
