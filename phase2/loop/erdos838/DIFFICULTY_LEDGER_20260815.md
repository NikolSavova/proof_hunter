...
| C2 | Threshold square-mesh Bellman inequality with harmonic loss | local bound loses only `O(log N log log(m+1))`; more generally, rows with largest/smallest child ratio at most `m^(1-delta)` give `log V >= (log N)^2/2-[(3/2-delta)/delta]q_*log N` under same-target inheritance and a final splice | ROUTE, not unrestricted gain | Dyadic local theorem and arbitrary-depth polynomial-imbalance theorem proved; only near-star scale separation, macroscopic arity, or geometric promotion can survive; `(WH)` is false | Any continuation must retain weighted predecessor identities in rows whose largest/smallest ratio is `m^(1-o(1))`; factor-two, fixed subcritical powers of `m`, Kraft lengths, and nested thresholds are closed |
...
| M5 | Uniform ordered `q`-block minimizer mutation | hoped-for stronger Gibbs/profile constraint beyond the physical bipartition | ROUTE / fixed-`q` BARRIER | Exact all-`q` rank kernel proved.  At endpoint rank at most `2 log n+O(1)`, every fixed `q>=4` returns the apparent Gibbs saving through its optional intermediate singletons; only `q=3` improves the extreme tail above `1.7095... log n`, and it still uses the same cap--cup reservoir | Do not pursue more uniform ordered blocks.  A continuation must be nonuniform, non-linear, or give an actual mixed-face decoder; see `agent_common_shield_mixing/POLYLOG_CAP_CUP_CONVERTER_MUTATION_GATE.md` (12e)--(12i) |
| M6 | Pointwise selected ordered three-block mutation | if every fixed-gap nonminimal state admitted a decreasing colouring, or stability forced a fixed-gain mixed reservoir, global minimality would close that state | BARRIER / CLOSED as an **unconditioned** uniform selector; the strict sub-half conditional remains a HOPE; **relation:** incomparable pointwise refinement of the averaged M5 kernel; **viability:** it improves the bipartition-stable eight-point trap by one and the Pascal wrapper by 445 | Exact controls pass, but cyclic and independently annealed low-face induced sets are stable under the stated complete/sparse three-colour audits; after one exact `n=20` singleton descent the annealed witness is again sparse-stable.  These controls lie above the half scale, so they do not refute a strict fixed-gap conditional theorem | Do not promote another fixed-`q` selector without a sub-half viability witness.  A continuation must be nonlinear/multichart, prove a genuinely fixed-gap-conditioned selector, or return to the direct fixed-rank gain; see `agent_mutation_spectrum_search/MUTATION_SPECTRUM_VIABILITY_AUDIT.md` |
| M7 | Strict-subhalf linear-pocket common-circuit core | every configuration with `log V<=(1/2-delta+o(1))L^2` contains two face families whose product is at least `V 2^((eta_delta/2-o(1))L^2)` and whose complete cross rectangle shares `Omega_delta(L)` fixed vertex-disjoint bad four-circuits; `eta_delta=delta-delta^2/2+delta^4/4` | STRICT structural reduction of the conditional M6 route; not itself a coefficient gain | Proved from the unconditional quarter lower bound, induced-subset source bank, exact deletion theorem, and literal localization; verifier PASS.  The fixed circuits are not a transversal and their own Boolean bank is only `2^O(L)` | Literal repair audited in M8. Continue only through positive-mass release or a decreasing mutation; the common-deletion target is closed. See `agent_common_shield_mixing/STRICT_SUBHALF_LINEAR_POCKET_CIRCUIT_CORE_GATE.md` |
| M8 | Common-circuit deletion cascade | after deleting a fixed core, released records inject into faces; otherwise one fixes a fresh residual circuit at cost `binom(n,4)`, giving `|A||H|<=2V(2 binom(n,4))^q` for residual two-sided rank `q` | ROUTE + BARRIER refining M7 | Exact theorem and verifier. In the true `n=9` minimizer, 1,569/10,800 fixed-circuit rectangles have no universal common-label deletion; the smallest is `2x8`. Full-core deletion nevertheless releases at least `25/28` in every audited rectangle | Close universal common deletion. Permit one bounded minimizer-specific positive-mass release/mutation attack; kill if it pays `n^4` independently per residual circuit or becomes unrestricted profile alignment. See `agent_common_shield_mixing/COMMON_CIRCUIT_DELETION_CASCADE_AND_MINIMIZER_GATE.md` |
...
| P1g | Binary drop-charged endpoint floor and a graded reset consequence | the proved local inequality gives `log(X_T(t)Y_T(t))>=phi(log(1+nt))` for every binary strong tree and `0<t<=1`; to affect 838 it would have to imply an averaged positive-rank interval or `v_k(4^k)>=2^((1+eta-o(1))k^2)` | BARRIER / CLOSED as a coefficient route; **relation:** the weighted endpoint theorem is strictly weaker than P1, while the missing graded consequence is P1d-shaped | Exact local theorem proved; graded propagation fails its viability test | At `n=4^k,t=2^-k`, a literal spine bank `(1+qz)^d` with `d=floor((k-1)/2)` has weighted logarithm `k^2/2-O(k log k)` but degree `<k`, so its rank-`k` coefficient is zero. Internal sibling faces require the original recursive rank-allocation gate. See `agent_weighted_endpoint_tilt/WEIGHTED_ENDPOINT_TILT_AND_RANK_EXTRACTION_BARRIER.md` |
...
3. an explicit predecessor classification: **strict reduction**,
   **equivalent reformulation**, or **incomparable new bet**;
4. a viability witness: the smallest finite or asymptotic instance on which
   the proposed mechanism is already true and produces an actual gain;
5. a counterexample class it must survive;
6. a kill condition.

If no viability witness is known, label the statement a **hope** rather
than a target and allow at most one bounded calibration attempt.  A finite
calibration must include relevant positive, minimal, and mutation-stable
nonminimal controls whenever those controls are available.
