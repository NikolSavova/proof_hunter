...
| M6 | Pointwise selected ordered three-block mutation | if every fixed-gap nonminimal state admitted a decreasing colouring, or stability forced a fixed-gain mixed reservoir, global minimality would close that state | BARRIER / CLOSED as a uniform selector; **relation:** incomparable pointwise refinement of the averaged M5 kernel; **viability:** it improves the bipartition-stable eight-point trap by one and the Pascal wrapper by 445 | Exact controls pass, but cyclic low-face induced sets at `n=10,12,14` are stable under every three-colouring; after one exact `n=20` singleton descent, all three-colourings moving at most four labels are stable | Do not promote another fixed-`q` selector.  A continuation must be nonlinear/multichart or return to the direct fixed-rank gain; see `agent_mutation_spectrum_search/MUTATION_SPECTRUM_VIABILITY_AUDIT.md` |
