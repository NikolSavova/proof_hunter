...
long double trace_at(int n, const std::vector<std::pair<int,int>>& roots,
                     long double z) {
...
Score evaluate(int n, const Word& w) {
  ...
  Score s;
  s.q1=trace_at(n,roots,1);
  s.qh=trace_at(n,roots,0.5L);
  s.f1=s.q1+1;
  s.fh=s.qh+1-n/2.0L;
  s.hq=n*s.qh/s.q1;
  s.hf=n*s.fh/s.f1;
...
int main(int argc,char**argv) {
  if (argc<6) {
    std::cerr << "usage: half_weight_search n steps restarts seed q|f [output] [coords]\n"
  ...
  bool use_q=std::string(argv[5])=="q";
  Word supplied;
  ...
  for (int restart=0;restart<restarts;++restart) {
    ...
    long double cv=use_q?cur.hq:cur.hf;
    ...
    for(long long step=0;step<steps;++step){
      ...
      if(cv>global){
        ...
        std::cerr<<"best n="<<n<<" r="<<restart<<" step="<<step
                 ...
                 <<" Hf="<<(double)cur.hf<<" q1="<<(double)cur.q1<<"\n";
      }
    }
  }
  ...
  *out<<"{\n  \"n\": "<<n<<",\n  \"steps_per_restart\": "<<steps
      <<",\n  \"restarts\": "<<restarts<<",\n  \"seed\": "<<seed
      <<",\n  \"objective\": \""<<(use_q?"matrix_trace":"convex_partition")<<"\",\n"
